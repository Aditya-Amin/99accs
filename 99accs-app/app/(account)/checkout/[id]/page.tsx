...
// Server-fetch the initial session envelope by calling our own mock API.
// This keeps the same shape we'll get from Laravel when wired up — the
// component layer treats this as a pre-rendered initial state.
async function fetchInitial(id: string): Promise<{
  ...
  const url = `${proto}://${host}/api/mock/checkout/${encodeURIComponent(id)}`;
...
export default async function CheckoutSessionPage({ params }: PageProps) {
  ...
  if (result.status === 'paid') redirect(`/order/${id}/confirmation`);
  ...
  if (result.envelope && result.envelope.session.status !== 'pending') {
    redirect(`/order/${id}/confirmation`);
  }
