...
export default async function OrderConfirmationPage({ params }: Props) {
  ...
  const envelope = await fetchOrder(order_number);
  if (!envelope) notFound();
  const { session } = envelope;
  const currency = session.currency;
  const orderShort = order_number.split('-')[0].toUpperCase();
            ...
            <div className="col-lg-9">
              <div className="order-confirm__card">
                ...
                <div className="order-confirm__actions">
                  ...
                  <Link href="/shop/valorant" className="border-btn order-confirm__shop-btn">Continue shopping</Link>
                </div>
              </div>
            </div>
          </div>
